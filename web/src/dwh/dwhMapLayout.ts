...
/** DWH radial edge: merkezlerden hesaplanan, düğüm sınırında başlayıp biten geometri. */
export function dwhRadialEdgeGeometry(
  ...
  const sourceBoundary = {
    x: sourceCenterX + ux * (sourceRadius + 2),
    y: sourceCenterY + uy * (sourceRadius + 2),
  }
  const targetBoundary = {
    x: targetCenterX - ux * (targetRadius + 6),
    y: targetCenterY - uy * (targetRadius + 6),
  }
  const lead = Math.max(18, Math.min(42, distance * 0.12))
  const sourceClear = {
    x: sourceBoundary.x + ux * lead,
    y: sourceBoundary.y + uy * lead,
  }
  const targetClear = {
    x: targetBoundary.x - ux * lead,
    y: targetBoundary.y - uy * lead,
  }
  const clearDx = targetClear.x - sourceClear.x
  const clearDy = targetClear.y - sourceClear.y
  const clearDistance = Math.hypot(clearDx, clearDy) || 1
  const bow = clearDistance * 0.09
  const control = {
    x: (sourceClear.x + targetClear.x) / 2 + nx * bow,
    y: (sourceClear.y + targetClear.y) / 2 + ny * bow,
  }
  return {
    path: [
      `M ${sourceBoundary.x},${sourceBoundary.y}`,
      `L ${sourceClear.x},${sourceClear.y}`,
      `Q ${control.x},${control.y} ${targetClear.x},${targetClear.y}`,
      `L ${targetBoundary.x},${targetBoundary.y}`,
    ].join(' '),
    mx: (sourceClear.x + 2 * control.x + targetClear.x) / 4,
    my: (sourceClear.y + 2 * control.y + targetClear.y) / 4,
    angle: Math.atan2(clearDy, clearDx),
  }
}
